...
Let's start with our old friend `data_dates`. So far, apart from some tidying up, we haven't tried to change this data. Let's change that now, starting with something straightforward: adding columns together using `mutate()`:
...
We've already met all the parts of this command before. We use the pipe `%>%` to pass the `data_dates` tibble to `mutate`. `mutate` creates a new column (`total`) by adding together the three `count` columns, and we then assign to a new tibble named `data_total`. 
...
This uses the `select` function to select columns. Here, the `!` means (roughly) "not" or "except". So `select(!count)` command means "select all columns except for count1. 
...
```{r}
data_total %>%
  summarise(start_date = min(date), end_date = max(date), total = sum(total))
```

Importantly, `summarise()` makes a new tibble based on the expressions we include within it. This example does three things inside `summarise()`:

+ finds the start date using `min(date)`
+ finds the end date using `max(date)`
+ then sums the total column using `sum(total)`

Note that we can call the new columns anything we like. So we could do the same summarising with different column names:

```{r}
data_total %>%
  summarise(beginnings = min(date), endings = max(date), grand_total = sum(total))
```

And we can do many summaries together:
...
- `across(where(is.numeric)` tells R that we want to use all the columns that contain numbers
- `~ sum(.)` tells summarise to use the following formula for each column: pass all the values (represented by `.`) into `sum()`
...
Number of days where count1 is greater than count3: `r data_dates %>% filter(count1 > count3) %>% nrow()` 

Count 2 was highest on this day of the week: `r data_dates %>% filter(count2==max(count2)) %>% pull(date) %>% wday(label=T, abbr=F)`
...
Sometimes our reports need to contain larger chunks of data than are comfortable to manage using inline text. The most obvious example is the data table. We have some R commands that are extremely helpful for writing data tables in reports. `kable` is the most useful of all, because it allows us to produce nicely formatted tables with the very minimum of fooling about. When we use `kable` to show the first few rows of the `data_dates` tibble we get the following:
