...
- `data_total |> ggplot()` - `ggplot2()` starts a new plot area, and we associate that new plot area with the `data_total` tibble using the pipe
...
+ Microsoft's PowerBI. This is more focused on building real-time dashboards, and it's intended to be a low-code way of working that should be a fairly natural step for staff with lots of Excel experience. We have [several training options](https://nes-dew.github.io/KIND-training/BI_main.html) for Power BI.
+ R + Shiny is the best R-based way of building real-time dashboards
